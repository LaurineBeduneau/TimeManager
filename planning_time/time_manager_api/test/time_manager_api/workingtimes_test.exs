defmodule TimeManagerApi.WorkingtimesTest do
  ...
  describe "workingtimes" do
    ...
    test "create_workingtime/1 with valid data creates a workingtime" do
      valid_attrs = %{start: ~U[2023-10-23 08:20:00Z], end: ~U[2023-10-23 08:20:00Z]}

      assert {:ok, %Workingtime{} = workingtime} = Workingtimes.create_workingtime(valid_attrs)
      assert workingtime.start == ~U[2023-10-23 08:20:00Z]
      assert workingtime.end == ~U[2023-10-23 08:20:00Z]
    end
    ...
    test "update_workingtime/2 with valid data updates the workingtime" do
      workingtime = workingtime_fixture()
      update_attrs = %{start: ~U[2023-10-24 08:20:00Z], end: ~U[2023-10-24 08:20:00Z]}

      assert {:ok, %Workingtime{} = workingtime} = Workingtimes.update_workingtime(workingtime, update_attrs)
      assert workingtime.start == ~U[2023-10-24 08:20:00Z]
      assert workingtime.end == ~U[2023-10-24 08:20:00Z]
    end
